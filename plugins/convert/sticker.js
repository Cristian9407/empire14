   ...
   async: async (m, {
      ...
      try {
         if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
            ...
            if (/video/.test(type) && (q.seconds > 10)) return client.reply(m.chat, Utils.texted('bold', `❌ La duración máxima del vídeo es de 10 segundos.`), m)
            ...
            if (q.mtype === 'albumMessage') {
               ...
               if (!result?.items?.length) return client.reply(m.chat, Utils.texted('bold', `❌ El mensaje del álbum no existe en la tienda. Vuelve a subirlo o reenvíalo para generar el paquete de stickers.`), m)
            ...
            if (/image\/(jpe?g|png)|video/.test(mime)) {
               ...
               if (getAssoc?.parentMessageKey?.id) {
                  ...
                  if (!result?.metaKnown)
                     m.reply(`⚠️ ADVERTENCIA: No se encontró el mensaje del álbum raíz en la tienda; se utilizará la estabilidad de reserva: ${result.rootId}`)
                  ...
                  m.reply(`El álbum está incompleto después del reintento: falta ${result.missing.image} image(s), ${result.missing.video} video(s)`)
               } else {
                  let buffer = null
                  ...
                  if (/image\/(jpe?g|png)/.test(mime)) {
                     ...
                     client.sendReact(m.chat, '🕒', m.key)
                     if ((q.msg || q).seconds > 10) return client.reply(m.chat, Utils.texted('bold', `❌ La duración máxima del vídeo es de 10 segundos.`), m)
                     buffer = await q.download()
