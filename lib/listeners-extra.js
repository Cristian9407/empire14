...
export default (system, client) => {
   try {
      ...
      client.register('group.add', async ctx => {
         ...
         const text = `Hola +tag Bienvenid@ a +grup`
         ...
         if (groupSet && groupSet.localonly) {
            if (global.db.users.some(v => v.jid == ctx.member) && !global.db.users.find(v => v.jid == ctx.member).whitelist && !ctx.member.startsWith('593') || !ctx.member.startsWith('593')) {
               sock.reply(ctx.jid, Utils.texted('bold', `Lo siento @${ctx.member.split`@`[0]}, Este grupo es solo para personas de Ecuador y serás eliminado automáticamente.`))
