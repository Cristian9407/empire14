...
export default (system, client, store) => {
   try {
      ...
      client.register('message', ctx => {
         ...
         if (store && !store.client) store.bind(ctx.store, client.sock)
      ...
      client.register('group.add', async ctx => {
         try {
            ...
            const text = `Hola *+tag* Bienvenido a *+grup*`
            ...
            if (groupSet && groupSet.localonly) {
               if (global.db.users.some(v => v.jid === memberId) && !global.db.users.find(v => v.jid == memberId).whitelist && !memberId.startsWith('593') || !memberId.startsWith('593')) {
                  sock.reply(ctx.jid, Utils.texted('bold', `Lo siento @${memberId.split`@`[0]}, este grupo es solo para personas Ecuatorianas y serás eliminado automáticamente.`))
