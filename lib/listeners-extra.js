...
export default (system, client) => {
   try {
         ...
         if (id.endsWith('g.us')) {
            ...
            for (let sender in presences) {
               ...
               if ((presence.lastKnownPresence === 'composing' || presence.lastKnownPresence === 'recording') && user.afk > -1) {
                  sock.reply(id, `El sistema detecta actividad de @${user.jid.replace(/@.+/, '')} después de estar desconectado por : ${Utils.texted('bold', Utils.toTime(new Date - (user?.afk || 0)))}\n\n➠ ${Utils.texted('bold', 'Reason')} : ${user?.afkReason || '-'}`, user?.afkObj)
                  user.afk = -1
                  user.afkReason = ''
                  user.afkObj = {}
               }
      ...
      client.register('group.add', async ctx => {
         ...
         const text = `Bienvenid@ +tag al grupo *+grup*`
         ...
         if (groupSet && groupSet.localonly) {
            if (global.db.users.some(v => v.jid === memberId) && !global.db.users.find(v => v.jid == memberId).whitelist && !memberId.startsWith('593') || !memberId.startsWith('593')) {
               sock.reply(ctx.jid, Utils.texted('bold', `Lo siento @${memberId.split`@`[0]}, este grupo es solo para personas de Ecuador y serás eliminado automáticamente.`))
