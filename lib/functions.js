...
Utils.greeting = () => {
   ...
   let res = `No olvides dormir`
   if (time >= 3) res = `Buenas días`
   if (time > 6) res = `Buenos días`
   if (time >= 11) res = `Buenas tardes`
...
Utils.hasRole = (m, users = []) => {
   ...
   return users
      ...
      .some(value => {
         ...
         const jid = Utils.validatePhone(valueString).valid
            ? `${valueString}@s.whatsapp.net`
            : `${valueString}@lid`
